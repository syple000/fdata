...
class UIAuto:
    ...
    def find_control_path(self, target_selector: ControlSelector, start_window=None) -> Optional[List[ControlSelector]]:
        ...
        try:
            ...
            if start_window is None:
                windows = self.app.windows()
                for window in windows:
                    path = self._search_control_in_tree(window, target_selector, [])
                    if path:
                        return path
                return None
            else:
    ...
    def _search_control_in_tree(self, control, target_selector: ControlSelector, current_path: List[ControlSelector]) -> Optional[List[ControlSelector]]:
        """在控件树中递归搜索匹配的控件"""
        try:
            # 检查当前控件是否匹配目标选择器
            if self._is_control_match(control, target_selector):
                # 创建当前控件的选择器
            ...
            try:
                ...
                for child in children:
                    ...
                    result = self._search_control_in_tree(child, target_selector, current_path + [current_selector])
                    if result:
                        return result
            except Exception as e:
                logging.debug(f"获取子控件失败: {str(e)}")
    ...
    def _is_control_match(self, control, selector: ControlSelector) -> bool:
        """检查控件是否匹配选择器条件"""
        try:
            # 检查每个非空的选择器条件
            if selector.title is not None:
                if control.window_text() != selector.title:
                    return False
            
            if selector.auto_id is not None:
                if not hasattr(control, 'automation_id') or control.automation_id() != selector.auto_id:
                    return False
            
            if selector.control_id is not None:
                if not hasattr(control, 'control_id') or control.control_id() != selector.control_id:
                    return False
            
            if selector.class_name is not None:
                if not hasattr(control, 'class_name') or control.class_name() != selector.class_name:
                    return False
            
            if selector.control_type is not None:
                if not hasattr(control, 'control_type') or control.control_type() != selector.control_type:
                    return False
...
if __name__ == '__main__':
    ...
    if ui_auto.connect(title_re='网上股票交易系统5.0'):
        ...
        control = ui_auto.get_control(ControlSelector(
            ...
            auto_id=None,
            control_id=None,
            class_name='Afx:009C0000:b:00010003:00000006:000108F7',
            control_type=None,
            found_index=None,
        ))
...
#        ])
        ...
        # 通过路径获取控件
        control3 = ui_auto.get_control_by_path(control, paths[1:4])
